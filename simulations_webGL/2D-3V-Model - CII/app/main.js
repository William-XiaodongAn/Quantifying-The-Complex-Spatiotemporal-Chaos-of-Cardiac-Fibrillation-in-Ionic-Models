 ...
 */
    ...
    env.outI     = new Abubu.FloatRenderTarget(will_length, will_length) ;
 ...
 */
    ...
    env.compTargets = function(_outVfs,_outI){
        this.outVfs = { location : 0  , target :  _outVfs     } ;
        this.outI = { location : 1  , target :  _outI         } ;
    } ;
    ...
    env.comp1 = new Abubu.Solver( {
        ...
        renderTargets   : new env.compTargets(  env.svfs, env.outI  ) ,
    } ) ;
    ...
    env.comp2 = new Abubu.Solver( {
        ...
        renderTargets   : new env.compTargets(  env.fvfs, env.outI    ) ,
    } ) ;
 ...
 */
	...
	var tau_pw_will = [350];
	var tau_si_will = [ [34.0,'tau_r'],[8.2,'tau_0'],[26.65,'tau_si'] ];
    


    var tau_pw_will_order = 0;
    var tau_si_will_order = 0;
	
    env.para_name = 'tau_d';
    env.para_value = 0.409;

    env[env.para_name] = env.para_value
    Abubu.setUniformInSolvers(env.para_name, env[env.para_name],[env.comp1,env.comp2 ]);
        ...
        if (env.running){
			for(var j=0 ; j< 2 ; j++){
				for(var i=0 ; i< env.frameRate/120 ; i++){	
					...
					if (env.time < 240000 && env.time > 200000 && countWill2%env.skip == 0 ){
						env.sampler.render() ;
					...
					if (env.time > 240200 && env.time < 240200 + 2.0*env.dt){ 

						saveCsvFile(willDataForOnePoint,env.para_name,env.para_value,'_spatial_20e4_32e4_8_8')
					...
					if (env.time > 254000 && tau_si_will_order != tau_si_will.length){
                        ...
                        env.para_name = tau_si_will[tau_si_will_order][1];
                        env.para_value = tau_si_will[tau_si_will_order][0];
...
function saveCsvFile(willDataForOnePoint, name, value, lya_type) {
	...
	var name_download = 'ICC_' + name +'_' + value + lya_type + '_APD_step_' + (2 * env.skip * env.dt) + '.csv';
