    ...
    env.render = function(){
        ...
        if (env.running){
			for(var i=0 ; i< env.frameRate/120 ; i++){	
				...
				if (env.time > 20000 && env.time < 100000 && countWill2%env.skip == 0 ){ //&& countWill2%20 == 0){ //20 * 2*dt = 20*2 * 0.1 = 4
				...
				if (env.time > 100200 && env.time < 100200 + 2.0*env.dt){

					saveCsvFile(willDataForOnePoint,env.tau_pw,env.tau_d,env.paramType,'_spatial_2e4_34e4_8_8')
...
function saveCsvFile(willDataForOnePoint,tau_pw,tau_d,paramType,lya_type){
	var link = document.createElement('a') ;

	var csv = encodeURI(  willDataForOnePoint ) ;
	link.setAttribute( 'href', csv ) ;
	
	var name_download = 'tau_pw_' + tau_pw +  '_tau_d_' + tau_d  + '_' + paramType + lya_type + '_APD_step_' + (2*env.skip*env.dt) + '.csv'
