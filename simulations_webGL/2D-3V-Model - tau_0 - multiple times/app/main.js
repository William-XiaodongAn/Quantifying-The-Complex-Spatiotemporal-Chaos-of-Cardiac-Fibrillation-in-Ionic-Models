 ...
 */
	...
	var tau_pw_will = [350];
	var tau_0_will = [7.2,7.4,7.6,7.8,8,8.2,8.4,8.6,8.8,9,9.2,9.4];

    env.tau_0 = 8.2			
    Abubu.setUniformInSolvers('tau_0', env.tau_0,[env.comp1,env.comp2 ]) ;	

    var tau_pw_will_order = 0;
    var tau_0_will_order = 0;
			...
			for(var j=0 ; j< 2 ; j++){
				for(var i=0 ; i< env.frameRate/120 ; i++){	
					...
					if (env.time > 520200 && env.time < 520200 + 2.0*env.dt){
						...
						saveCsvFile(willDataForOnePoint,env.tau_pw,env.tau_0,'_spatial_20e4_32e4_8_8')
					...
					if (env.time > 1830000 && env.time < 1830000 + 2.0*env.dt){
						...
						console.log('current time = ' + env.time + 'tau_pw_' + env.tau_pw +  '_tau_0_' + env.tau_0 + '\n')
					...
					if (env.time > 1850200 && tau_0_will_order != tau_0_will.length){
						...
						env.tau_0 = tau_0_will[tau_0_will_order]
...
function saveCsvFile(willDataForOnePoint, tau_pw, tau_d, lya_type) {
	...
	var name_download = 'tau_pw_' + tau_pw + '_tau_0_' + tau_d + lya_type + '_APD_step_' + (2 * env.skip * env.dt) + '.csv';
