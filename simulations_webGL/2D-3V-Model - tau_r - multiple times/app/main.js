 ...
 */
	...
	var tau_pw_will = [350];
	var tau_r_will  = [33.5,33.6,33.7,33.8,33.9,34,34.1,34.2,34.3,34.4,34.5];
	
    var tau_pw_will_order = 0;
    var tau_r_will_order = 0;
			...
			for(var j=0 ; j< 2 ; j++){
				for(var i=0 ; i< env.frameRate/120 ; i++){	
					...
					if (env.time > 520200 && env.time < 520200 + 2.0*env.dt){
						...
						saveCsvFile(willDataForOnePoint,env.tau_pw,env.tau_r,'_spatial_20e4_32e4_8_8')
					...
					if (env.time > 1830000 && env.time < 1830000 + 2.0*env.dt){
						...
						console.log('current time = ' + env.time + 'tau_pw_' + env.tau_pw +  '_tau_r_' + env.tau_r + '\n')
					...
					if (env.time > 1850200 && tau_r_will_order != tau_r_will.length){
						...
						env.tau_r = tau_r_will[tau_r_will_order]
						
						Abubu.setUniformInSolvers('tau_pw', env.tau_pw,[env.comp1,env.comp2 ]) ;						
						Abubu.setUniformInSolvers('tau_r', env.tau_r,[env.comp1,env.comp2 ]) ;
...
function saveCsvFile(willDataForOnePoint, tau_pw, tau_d, lya_type) {
	...
	var name_download = 'tau_pw_' + tau_pw + '_tau_r_' + tau_d + lya_type + '_APD_step_' + (2 * env.skip * env.dt) + '.csv';
